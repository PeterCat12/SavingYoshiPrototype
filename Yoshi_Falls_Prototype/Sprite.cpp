...
Sprite::Sprite(Bitmap* pBitmap)
{
  ...
  m_iZOrder = 0;
  SetRect(&m_rcBounds, 0, 0, 640, 480);  //Default rcBounds... I think that's stupid? 
...
//Swaps the sprites current Bitmap with another one, if the bitmaps are a different size, then
//the sprites collision/position rects need to be recalculated
void Sprite::SwapBitmap(Bitmap& pBitmap)
{
	if (m_pBitmap->GetWidth() != pBitmap.GetWidth() ||
		m_pBitmap->GetHeight() != pBitmap.GetHeight())
	{
		// swap sprite and recalculate position/collision
		m_pBitmap = &pBitmap;
		//Here I should code for recalculations...
	}
	else
	{
		//The bitmaps are the same size, so no need to recalculate
		m_pBitmap = &pBitmap;
	}
	//ResetFrameInfo(); // Reset frame information so the frame draws from the begining.
}
...
//Resets the frame information so the sprite knows to start drawing at the begining of a new Bitmap
//DOES NOT tamper with frame delay, might require a fix later...
void Sprite::ResetFrameInfo()
{
	m_iCurFrame = 0;
	m_iFrameTrigger = 0;
}