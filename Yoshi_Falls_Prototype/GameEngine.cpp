      ...
      else
      {
        ...
        if (!GameEngine::GetEngine()->GetSleep())
        {
          ...
          if (iTickCount > iTickTrigger)
          {
            ...
            HandleKeys();
            GameEngine::GetEngine()->CheckJoystick();  //Basically the same as handleKeys() but for joysticks.
            GameCycle();
          }
        }
      }
...
// I assume this is the kind of shit he was talking about?? 
  ...
  LPTSTR szTitle, WORD wIcon, WORD wSmallIcon, int iWidth, int iHeight)
{
  ...
  g_pBitmapMan = new BitmapManager();
  m_uiMIDIPlayerID = 0;
}
...
void GameEngine::AddBitmap(Bitmap& pBitmap)
{
	g_pBitmapMan->addBitmap(pBitmap);
}
